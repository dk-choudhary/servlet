...
@WebServlet("/showAllAc")
public class ShowAllAccountBankEmp extends HttpServlet {
@Override
protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    try {
		...
		PreparedStatement ps=con.prepareStatement("select count(*) from customer_basic_deatils");
	...
	String page="<!DOCTYPE html>\r\n"
			...
			while(totalRow>0) {
				...
				page=page+"<a href='showAllAc?page="+(pageLink)+"'>"+(pageLink++)+"</a>\t";
			}
			page=page+ "	</div>\r\n"
					+ "\r\n"
			+ "	<footer>\r\n"
 ...
 public List<ArrayList> getTransction(int start,int total,HttpSession session,ServletContext context){
	 List<ArrayList> list=new ArrayList<ArrayList>();
	    try {
			Connection con=(Connection) context.getAttribute("connection");
			PreparedStatement ps=con.prepareStatement("select ac_number,name,ac_type,current_balance from customer_basic_deatils where task_panding<1 limit "+(start-1)+","+total);
			ResultSet rs=ps.executeQuery();
		    while(rs.next()) {
		    	...
		    	arrayList.add(rs.getLong("current_balance"));
		    	list.add(arrayList);
		    }
